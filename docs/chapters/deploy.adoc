...
`bin/jboss-cli.sh` or `bin/jboss-cli.bat` is Command Line Interface management tool for a standalone server or managed domain. It allows a user to connect to a standalone server or domain controller and execute management operations.
...
. Start jboss-cli and connect with the existing standalone instance by giving the following command:
+
[source]
----
jboss-cli.sh -c
...
+
TIP: `--force` switch can be specified to redeploy the application.
+
...
[source]
----
jboss-cli.sh --connect --command="deploy target/javaee7-sample.war --force"
----
