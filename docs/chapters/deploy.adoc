...
. Create a Java class and name it `MyDeployer`.
+
. In that Java class, create a method as shown:
+
[source]
----
...
In this code:
+
.. `ModelControllerClient` is client for an application server management model controller.
.. A new instance is created by specifying host, port, and a Callback handler to obtain authentication information for the call.
+
. Add another method to this class as:
+
[source]
----
private void deploy() throws IOException, InterruptedException, ExecutionException {
    // create client
...
----
+
Deployment, as earlier, is a two-step process. First step require to upload the archive to content repository. Second step involves deploying that archive to the application server instance.
In this code:
+
...
TIP: `addDeploy` method may be replaced with `andReplace` to replace a previously deployed archive.
